 ...
 *
*/
#ifndef IGNITION_GAZEBO_DETAIL_COMPONENTSTORAGE_HH_
#define IGNITION_GAZEBO_DETAIL_COMPONENTSTORAGE_HH_
...
{
  /// \brief The type of the component added to an entity is a type that wasn't
  /// previously associated with the entity.
  NEW_ADDITION = 0,
  ...
  /// already has a valid instance of this component type. Instead, the data for
  /// the valid instance was modified with the new component data
  MODIFICATION = 2,
...
class IGNITION_GAZEBO_HIDDEN ComponentStorage
{
  ...
  /// \return True if _entity was succesfully added to the component storage.
  /// False if _entity already belongs to the component storage
  ...
  /// \param[in] _entity The entity
  /// \return True if _entity was in the component storage and removed. False if
  /// _entity was not in the component storage
              ...
              const ComponentTypeT &_component)
  {
    ...
    // modifying the data of the pre-existing component
  ...
  /// \param[in] _entity The entity
  /// \return True if _entity exists and a component of type _typeId was removed
  ...
  /// \sa ValidComponent
  public: const components::BaseComponent *Component(const Entity _entity,
              const ComponentTypeId _typeId) const;

  // non-const version of the Component method
  public: components::BaseComponent *Component(const Entity _entity,
              const ComponentTypeId _typeId);
  ...
  /// should be checked to learn more about the state/validity of the component.
  /// \param[in] _entity The entity
  /// \return The pointer to a component type, if it exists. Otherwise, nullptr.
  /// nullptr is also returned if _entity does not exist
  /// \sa ValidComponent
  public: template<typename ComponentTypeT>
  ComponentTypeT *Component(const Entity _entity) const
  {
    const auto baseComponentPtr =
      this->Component(_entity, ComponentTypeT::typeId);
    if (nullptr == baseComponentPtr)
      return nullptr;
    return static_cast<ComponentTypeT*>(baseComponentPtr);
  }
  ...
  // non-const version of the ValidComponent method
  ...
  /// is returned
  public: template<typename ComponentTypeT>
  const ComponentTypeT *ValidComponent(const Entity _entity) const
  {
  ...
  // non-const version of the ValidComponent method
  public: template<typename ComponentTypeT>
  ComponentTypeT *ValidComponent(const Entity _entity)
  {
    auto compPtr = this->Component<ComponentTypeT>(_entity);
    if (nullptr != compPtr && !compPtr->ignore)
      return compPtr;
