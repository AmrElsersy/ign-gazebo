    ...
    const Entity _entity, const ComponentKey &_key)
{
  ...
  auto oneTimeIter = this->dataPtr->oneTimeChangedComponents.find(_key.first);
  if (oneTimeIter != this->dataPtr->oneTimeChangedComponents.end())
  {
    oneTimeIter->second.erase(_entity);
    if (oneTimeIter->second.empty())
      this->dataPtr->oneTimeChangedComponents.erase(_key.first);
  }

  auto periodicIter = this->dataPtr->periodicChangedComponents.find(_key.first);
  if (periodicIter != this->dataPtr->periodicChangedComponents.end())
  {
    periodicIter->second.erase(_entity);
    if (periodicIter->second.empty())
      this->dataPtr->periodicChangedComponents.erase(_key.first);
  }

  auto removedComp =
    this->dataPtr->componentStorage.RemoveComponent(_entity, _key.first);
  if (removedComp)
  {
    // update views to reflect the component removal
    for (auto &viewPair : this->dataPtr->views)
      viewPair.second->NotifyComponentRemoval(_entity, _key.first);
  }
  ...
  {
    std::lock_guard<std::mutex> lock(this->dataPtr->removedComponentsMutex);
    this->dataPtr->removedComponents[_entity].insert(_key.first);
  }
...
ComponentKey EntityComponentManager::CreateComponentImplementation(
    ...
    const components::BaseComponent *_data, bool &_updateData)
{
  // TODO double check the case of calling CreateComponent on an entity that
  // already has a component of the requested type... it looks like we don't
  // check for this in the current ECM implementation; we just create a new one
      ...
      !components::Factory::Instance()->HasType(_componentTypeId))
  {
    ...
    // TODO(adlarkin) return something else? This is a private method, I believe the return
    // type can be changed without deprecation
    return ComponentKey();
  }
  ...
  // TODO(adlarkin) return something else? This is a private method, I believe the return
  // type can be changed without deprecation (I need to see if this is used by any
  // public methods first)
  ComponentKey compKey;
  compKey.first = _componentTypeId;
  return compKey;
}
...
const components::BaseComponent
    *EntityComponentManager::ComponentImplementation(
    const ComponentKey &/*_key*/) const
{
  // TODO(adlarkin) look at other TODO note in the following method
 ...
 */
  ...
  // commenting this out for now so code compiles (this is the previous implementation)
  /*
...
{
  // TODO(adlarkin) update componentStorage to implement this functionality
  // (temporary return nullptr to achieve compilation)
  return nullptr;
}
    ...
    for (const auto &compIter : iter.second.components())
    {
      ...
      if (nullptr == comp)
      {
        ...
        this->CreateComponentImplementation(entity,
            newComp->TypeId(), newComp.get(), temp);
      }
      // Update component value
      else
      {
